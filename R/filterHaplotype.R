...
createHaplotypOverviewTable <- function(allHaplotypesFilenames, clusterFilenames, chimeraFilenames, 
                                        referenceSequence=NULL, snpList, verbose=FALSE, max_indel_threshold=3) {
  ...
  if(!is.null(referenceSequence)){
		...
		idx <- (S4Vectors::lengths(deletion(aln1)) + S4Vectors::lengths(insertion(aln1))) <= max_indel_threshold 
